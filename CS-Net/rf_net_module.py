...
class RFNetModule(nn.Module):
    ...
    def criterion(self, endpoint):
        ...
        im1_topkmask = endpoint["im1_topkmask"]
        im2_topkmask = endpoint["im2_topkmask"]
        #
        # score loss
        #
        ...
        #
        # score rank consistency  loss
        #
        im1_scloss_cosis = self.det.loss_cosis(im1_score, im1_gtsc, im1_visible, im1_topkmask)
        im2_scloss_cosis = self.det.loss_cosis(im2_score, im2_gtsc, im2_visible, im2_topkmask)
        score_cosis_loss = (im1_scloss_cosis + im2_scloss_cosis) / 2.0 
        #
        # score discriminative rank loss
        #
        im1_scloss_discrim, im1_discrim_rank = self.det.loss_discrim(im1_score, im1_topkmask, im1_lpdes, im1_rpdes, im1_limc, im1_rimcw)
        im2_scloss_discrim, im2_discrim_rank= self.det.loss_discrim(im2_score, im2_topkmask, im2_lpdes, im2_rpdes, im2_limc, im2_rimcw)
        score_discrim_loss = (im1_scloss_discrim + im2_scloss_discrim) / 2.0 * 100
        score_discrim_rank_cosis_loss = (im1_discrim_rank + im2_discrim_rank) / 2.0


 #
        # score discriminative_triplet rank loss
        #
        im1_scloss_discrim_triplet, im1_discrim_rank_triplet = self.det.loss_discrim_triplet(im1_score, im1_topkmask, im1_lpdes, im1_rpdes, im1_limc, im1_rimcw)
        im2_scloss_discrim_triplet, im2_discrim_rank_triplet= self.det.loss_discrim_triplet(im2_score, im2_topkmask, im2_lpdes, im2_rpdes, im2_limc, im2_rimcw)
        score_discrim_loss_triplet = (im1_scloss_discrim_triplet + im2_scloss_discrim_triplet) / 2.0 *  100
        score_discrim_rank_cosis_loss_triplet = (im1_discrim_rank_triplet + im2_discrim_rank_triplet) / 2.0 
        ...
        # score discriminative rank loss
        #
        im1_scloss_discrim_des, im1_discrim_rank_des = self.des.loss_discrim(im1_score, im1_topkmask, im1_lpdes, im1_rpdes, im1_limc, im1_rimcw)
        im2_scloss_discrim_des, im2_discrim_rank_des= self.des.loss_discrim(im2_score, im2_topkmask, im2_lpdes, im2_rpdes, im2_limc, im2_rimcw)
        score_discrim_des_loss = (im1_scloss_discrim_des+ im2_scloss_discrim_des) / 2.0 *  100
        score_discrim_rank_cosis_des_loss = (im1_discrim_rank_des + im2_discrim_rank_des) / 2.0 

        # loss summary
        det_loss = score_cosis_loss + pair_loss + score_discrim_rank_cosis_loss
        des_loss = hard_loss + score_discrim_rank_cosis_des_loss

        PLT_SCALAR = {}
        PLT = {"scalar": PLT_SCALAR}

        PLT_SCALAR["score_loss"] = score_loss
        PLT_SCALAR["score_cosis_loss"] = score_cosis_loss
        PLT_SCALAR["score_discrim_loss"] =score_discrim_loss
        PLT_SCALAR["score_discrim_rank_cosis_loss"] = score_discrim_rank_cosis_loss
